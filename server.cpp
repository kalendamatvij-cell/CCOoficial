...
<head>
    ...
    <title>ССО - Статут та Правила (C++)</title>
...
<body class="gradient-bg min-h-screen text-gray-100">
        ...
        <div class="w-full px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center">
                    ...
                    <span class="ml-2 text-sm text-gray-400">Статут (C++)</span>
                </div>
            </div>
        </div>
    </nav>
    <header class="pt-32 pb-20 px-4">
        <div class="w-full text-center">
            <h1 class="text-6xl md:text-7xl font-extrabold text-white mb-6">
                <span class="bg-gradient-to-r from-blue-400 via-purple-400 to-pink-500 bg-clip-text text-transparent">ССО</span>
                <span class="text-white"> Статут</span>
            </h1>
            <p class="text-xl text-gray-300">C++ Version</p>
        </div>
    </header>
    <main class="w-full px-4 sm:px-6 lg:px-8 pb-20">
        <div class="bg-gradient-to-br from-slate-800/70 to-slate-900/70 rounded-2xl p-6 md:p-8 border border-slate-700/50 card-hover backdrop-blur-sm">
            <h2 class="text-2xl font-bold text-white mb-4">⚙️ C++ Version</h2>
            <p class="text-gray-300">Ця версія сайту працює на C++. Для запуску потрібен HTTP сервер (наприклад, Crow або cpp-httplib).</p>
            <p class="text-gray-400 mt-4 text-sm">Для запуску з cpp-httplib:</p>
            <pre class="bg-slate-900 rounded-lg p-4 mt-2 text-sm text-green-400 overflow-x-auto"><code>g++ server.cpp -lhttplib -lpthread -o server && ./server</code></pre>
        </div>
    </main>
</body>
</html>
